...
exports.startNewsWorker = startNewsWorker;
exports.stopNewsWorker = stopNewsWorker;
...
async function fetchAndSaveNews(prisma) {
    console.log('[NewsWorker] Starting news fetch...');
    let allArticles = [];
    const feedPromises = RSS_FEEDS.map(feed => parser.parseURL(feed.url)
    ...
    for (const article of allArticles) {
        ...
        if (!isDuplicate) {
            const dbDuplicate = existingNews.find(e => getSimilarity(article.title, e.title) > SIMILARITY_THRESHOLD);
            if (!dbDuplicate) {
...
let intervalId = null;
function startNewsWorker(prisma, intervalMinutes = 15) {
